...
from watercloudmodel_vwc import cost_function_vwc, ssrt_jac_vwc, ssrt_vwc
...
def do_one_pixel_field(data_field, vv, vh, vwc, theta, time, sm, sm_std, b, b_std, omega, rms, orbits, unc):
    ...
    # for jj in range(len(vv)):
        ...
        prior_mean = np.concatenate([[0,   ]*2, osm,     ovwc])
        prior_unc  = np.concatenate([[10., ]*2, osm_std, ovwc_std])

        xvv = np.array([rms, omega])


        x0 = np.concatenate([xvv, osm, ovwc])
                        ...
                        for kkkk in field:
                            for k in surface_list:
                                for kk in canopy_list:
                                    ...
                                    vwc_data = pd.read_csv('/media/tweiss/Work/z_final_mni_data_2017/vwc_sentinel_2'+pp+'_'+year+'_paper3_gao.csv', header=[0,1],index_col=0)
                                    ...
                                    vwc_field = vwc_data.filter(like=kkkk).filter(like=').1')
                                    ...
                                    # pdb.set_trace()
                                    times, vwcs, bs, sms, ps, orbit_mask = do_one_pixel_field(data_field, vv, vh, vwc, theta, time, sm, sm_std, b, b_std, omega, rms, orbits,unc=unc)

                                    fig, ax = plt.subplots(figsize=(17, 13))
                                    gs = gridspec.GridSpec(4, 1, height_ratios=[5, 5, 5, 5])
                                    ax = plt.subplot(gs[0])
                                    ...
                                    ubrmse_vv = ubrmse_prediction(rmse_vv,bias_vv)
                                    plt.title('rms: 0.013, rms model:'+str(ps[0,0])[:6]+' omega: 0.0107, omega model:'+str(ps[0,1])[:6])



                                    # plt.show()
                                    plt.savefig('/media/tweiss/Work/paper3/plot/'+year[1:]+'/vwc/'+kkkk, bbox_inches = 'tight')

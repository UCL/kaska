...
def smoothn(y, nS0=10, axis=None, smoothOrder=2.0, sd=None, verbose=False,
    ...
    for i in axis:
        ...
        Lambda = Lambda + \
            (cos(pi*(arange(1, sizy[i]+1) - 1.)/sizy[i]).reshape(siz0))
        # else:
        #   Lambda = Lambda + siz0
    ...
    try:
        sMinBnd = np.sqrt(
          (((1+sqrt(1+8*hMax**(2./N)))/4./hMax**(2./N))**2-1)/16.
          )
        sMaxBnd = np.sqrt(
    ...
    while RobustIterativeProcess:
        ...
        while tol > TolZ and nit < MaxIter:
            ...
            if isauto and not remainder(log2(nit), 1):
                ...
                # xpost,f,d =
                #   lbfgsb.fmin_l_bfgs_b(gcv,xpost,fprime=None,factr=10.,
                ...
                # only need to do it once though.
                # nS0 is teh number of samples used
                if not s0:
                    ss = np.arange(nS0) * \
                        (1./(nS0-1.))*(log10(sMaxBnd)-log10(sMinBnd)) + \
                        log10(sMinBnd)
                    g = np.zeros_like(ss)
                    for i, p in enumerate(ss):
                        g[i] = gcv(p, Lambda, aow, DCTy, IsFinite,
                                   Wtot, y, nof, noe, smoothOrder)
                        # print 10**p,g[i]
                    xpost = [ss[g == g.min()]]
                    # print '==============='
                    ...
                    xpost = [s0]
                xpost, f, d = \
                    lbfgsb.fmin_l_bfgs_b(gcv, xpost, fprime=None,
                                         factr=1e7,
                                         approx_grad=True,
                                         bounds=[(log10(sMinBnd),
                                                 log10(sMaxBnd))],
                                         args=(Lambda, aow, DCTy,
                                               IsFinite, Wtot, y,
                                               nof, noe, smoothOrder))
            s = 10**xpost[0]
        ...
        if isrobust:  # -- Robust Smoothing: iteratively re-weighted process
            ...
            RobustIterativeProcess = RobustStep < 3  # 3 robust steps are
                                                     # enough
        else:
    ...
    if isauto:
        if abs(log10(s)-log10(sMinBnd)) < errp:
            warning('MATLAB:smoothn:SLowerBound',
                    ['s = %.3f ' %(s) + ': the lower bound for s has been '
                        + 'reached. Put s as an input variable if required.'])
        elif abs(log10(s)-log10(sMaxBnd)) < errp:
            warning('MATLAB:smoothn:SUpperBound',
                    ['s = %.3f ' %(s) + ': the upper bound for s has been '
                        + 'reached. Put s as an input variable if required.'])
        # warning('MATLAB:smoothn:MaxIter',
        #   ['Maximum number of iterations (%d'%(MaxIter) + ') has been '
        #    + 'exceeded. Increase MaxIter option or decrease TolZ value.'])
    return z, s, exitflag, Wtot
...
def InitialGuess(y, I):
    # -- nearest neighbour interpolation (in case of missing values)
    if any(~I):
        try:

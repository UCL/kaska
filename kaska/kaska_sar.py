...
if __name__ == "__main__":
    ...
    s2_doys = [
        int(dt.datetime.strftime(x, "%j")) for x in s2_data.f.temporal_grid
    ]
    s1_doys = [int(dt.datetime.strftime(x, "%j")) for x in s1_obs.dates.keys()]
    ...
    def cost_nolai(xx, svh, svv, lai, cab, theta):
        n_obs = len(svh)
        return cost(np.concatenate([xx[:6], lai, lai, xx[-n_obs:]]), svh, svv,
                    theta)

    def cost_nolai_jac(xx, svh, svv, lai, cab, theta):
        n_obs = len(svh)
        return cost_jac(np.concatenate([xx[:6], lai, lai, xx[-n_obs:]]), svh,
                        svv, theta)

    def cost_nolai_hess(xx, svh, svv, lai, cab, theta):
        n_obs = len(svh)
        return cost_hess(np.concatenate([xx[:6], lai, lai, xx[-n_obs:]]), svh,
                         svv, theta)
    ...
    tic = time.time()
    n_pxls = 0
    bounds = [[-40, -5], [1e-4, 1], [-40, -1], [-40, -5], [1e-4, 1], [-40, -1],
              *([[0.01, 1]] * n_sar_obs)]
    x0 = x0_all
    for (row, col) in np.ndindex(*lai_s1[0].shape):
        ...
        retval = scipy.optimize.minimize(cost_nolai,
                                         x0,

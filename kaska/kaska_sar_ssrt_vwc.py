...
from watercloudmodel_vwc import cost_function_vwc
                        ...
                        for pp in aggregation:
                            ...
                            for i, ii in enumerate(versions):
                                ...
                                if ii == 'everything':
                                    ...
                                    orbit3=None
                                    orbit4=None
                                    plot_output_path = '/media/tweiss/Work/paper3/z_dense_s1_time_series_n1'+p+pp+'_all'+'/'
                                    ...
                                    orbit3=None
                                    orbit4=None
                                    plot_output_path = '/media/tweiss/Work/paper3/z_dense_s1_time_series_n1'+p+pp+'/'
                                    csv_output_path = plot_output_path+'csv/'
                                else:
                                    plot_output_path = '/media/tweiss/Work/paper3/z_dense_s1_time_series_n1'+p+pp+'_'+ii+'/'
                                ...
                                for kkk in opt_mod:
                                    for k in surface_list:
                                        for kk in canopy_list:
                                            ###################
                                            field3 = field2[0:4]+'high'
                                            data_field =data.filter(like=k).filter(like=kk).filter(like=kkk).filter(like=field3)
                                            #########################
                                            ...
                                            for z in range(len(state_mask)):
                                                for zz in range(len(state_mask[0])):
                                                    if state_mask[z,zz] == False:
                                                        ...
                                                        times, vwcs, bs, sms, ps, orbit_mask = do_one_pixel_field(data_field, vv, vh, vwc, theta, time, sm, sm_std, coef, coef_std, omega, rms, orbits,unc=unc)
                                            ...
                                            for u in range(len(sm_retrieved)):
                                                ...
                                                plt.savefig('/media/tweiss/Daten/data_AGU/test_kaska/down2/542_'+str(time[u])[:10], bbox_inches = 'tight')
                                                plt.close()

...
def define_temporal_grid(start_date, end_date, temporal_grid_space):
    ...
    temporal_grid = [
        start_date + i * dt.timedelta(days=temporal_grid_space)
        for i in range(int(np.ceil(366 / temporal_grid_space)))
        if start_date + i * dt.timedelta(days=temporal_grid_space) <= end_date
    ]
...
def reproject_data(
    ...
    if srcNodata is None:
        try:
            srcNodata = " ".join([
                i.split("=")[1] for i in gdal.Info(source_img).split("\n")
                if " NoData" in i
            ])
        except RuntimeError:
            srcNodata = None
    ...
    if (target_img is None) & (dstSRS is None):
        raise IOError("Projection should be specified ether from " +
...
def save_output_parameters(
    ...
    for (param, data) in zip(parameter_names, output_data):
        for band, tstep in enumerate(time_grid):
            ...
            dst_ds = drv.Create(str(outfile), nx, ny, 1, gdal.GDT_Float32,
                                options)
...
def rasterise_vector(vector_f, sample_f=None, pixel_size=20):
    ...
    target_ds = gdal.GetDriverByName("MEM").Create("", cols, rows, 1,
                                                   gdal.GDT_Byte)

...
S1data = namedtuple(
    ...
    "S1data",
    "time VV VH theta VV_unc VH_unc")

layers = [
    "sigma0_vv_norm_multi_db", "sigma0_vh_norm_multi_db", "localIncidenceAngle"
]
...
def get_s1_dates(s1_file):
    ...
    times = [
        float(s1_file.GetRasterBand(b + 1).GetMetadata()['NETCDF_DIM_time'])
...
class Sentinel1Observations(object):
    ...
    def _match_to_mask(self):
        ...
        for layer, layer_name in self.nc_layers.items():
            ...
            self.s1_data_ptr[layer] = reproject_data(
                fname,
                output_format="VRT",
                srcSRS="EPSG:4326",
        ...
        self.dates = {
            ...
            if ((x >= self.time_grid[0]) and (x <= self.time_grid[-1]))
        }
    ...
    def read_time_series(self, time_grid):
        ...
        sel_dates = [
            k for k, v in self.dates.items() if ((k >= early) and (k < late))
        ]
        sel_bands = [
            v for k, v in self.dates.items() if ((k >= early) and (k < late))
        ]
        obs = {}
        for ii, layer in enumerate(self.s1_data_ptr.keys()):
            obs[layer] = np.array([
                self.s1_data_ptr[layer].GetRasterBand(i).ReadAsArray()
                for i in sel_bands
            ])
        the_obs = S1data(sel_dates, obs['VV'], obs['VH'], obs['theta'], 0.5,
                         0.5)
        return the_obs

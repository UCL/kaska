...
# def do_one_pixel_field(data_field, vv, vh, lai, theta, time, sm, sm_std, sr, sr_std, height, orbits, unc):
...
aggregation = ['_buffer_100','_Field_buffer_30']
    ...
    for pp in aggregation:
        ...
        for i, ii in enumerate(versions):
            ...
            if ii == 'everything':
                ...
                orbit3=None
                orbit4=None
                plot_output_path = '/media/tweiss/Work/paper3/z_dense_s1_time_series_n7'+p+pp+'_all'+'/'
                ...
                orbit3=None
                orbit4=None
                plot_output_path = '/media/tweiss/Work/paper3/z_dense_s1_time_series_n7'+p+pp+'/'
                csv_output_path = plot_output_path+'csv/'
            else:
                plot_output_path = '/media/tweiss/Work/paper3/z_dense_s1_time_series_n7'+p+pp+'_'+ii+'/'
                ...
                for kkkk in field:
                    for k in surface_list:
                        for kk in canopy_list:
                            ...
                            vv = 10*np.log10(vv)
                            vv_model = data_field.filter(like='vv_model').values.flatten()
                            vv_model = 10*np.log10(vv_model)
                            ...
                            tau = data_field.filter(like='coef').values.flatten() * np.sqrt(data_field.filter(like='LAI').values.flatten()) * data_field.filter(like='height').values.flatten()
                            height = data_field.filter(like='height').values.flatten()


                            data_biomass = pd.read_csv('/media/tweiss/Work/z_final_mni_data_2017/new_in_situ_s1multi_buffer_50neu_biomass.csv',header=[0,1],index_col=1)

                            data_biomass = data_biomass[:len(height)]
                            data_biomass2 = data_biomass.filter(like=kkkk)
                            wetbiomass = data_biomass2.filter(like='wet')
                            drybiomass = data_biomass2.filter(like='dry')


                            xxx = tau[mask168]-tau[mask117]

                            # plt.rcParams["figure.figsize"] = (10,7)
                            # plt.plot(time.values, tau)
                            # plt.grid()
                            # plt.ylabel('tau')
                            # plt.xlabel('Time')
                            # plt.xlim([datetime.date(2017, 3, 22), datetime.date(2017, 7, 18)])
                            # # plt.savefig('/media/tweiss/Daten/data_AGU/test_kaska/tau_x', bbox_inches = 'tight')
                            ...
                            vwc = data_field.filter(like='VWC').values.flatten()
                            b = tau / vwc
                            coef = data_field.filter(like='coef').values.flatten()
                            # plt.plot(time.values, b)
                            # plt.grid()
                            # plt.ylabel('b = tau/vwc')
                            # plt.xlabel('Time')
                            # plt.xlim([datetime.date(2017, 3, 22), datetime.date(2017, 7, 18)])
                            # plt.savefig('/media/tweiss/Daten/data_AGU/test_kaska/b_301_n1', bbox_inches = 'tight')
                            # pdb.set_trace()


                            fig, ax = plt.subplots(figsize=(17, 13))
                            gs = gridspec.GridSpec(7, 1, height_ratios=[5, 5, 5, 5, 5,5,5])
                            ax = plt.subplot(gs[0])

                            plt.ylabel('Backscatter [dB]', fontsize=14)
                            # plt.xlabel('Date', fontsize=18)
                            # plt.tick_params(labelsize=12)

                            ax.plot(time.values, vv, label='VV S1')
                            ax.plot(time.values, vv_model, label='VV model')
                            ...
                            ax1.plot(time.values, vwc, label='VWC')
                            plt.ylabel('VWC [kg/m2]', fontsize=14)
                            plt.grid(linestyle='dotted')
                            ax.set_xlim([datetime.date(2017, 3, 22), datetime.date(2017, 7, 18)])
                            plt.subplots_adjust(hspace=.0)

                            ax2 = plt.subplot(gs[2])
                            ax2.plot(time.values, tau, label='tau')
                            ...
                            ax3.plot(time.values, b, label='b=tau/vwc')
                            ...
                            ax4.plot(time.values, coef, label='coef')
                            ...
                            ax5.plot(time.values, height, label='height')
                            ...
                            ax6.plot(time.values, wetbiomass, label='wet biomass')
                            ax6.plot(time.values, drybiomass, label='dry biomass')
                            plt.ylabel('Biomass [kg/m2]', fontsize=14)
                            plt.grid(linestyle='dotted')
                            plt.legend()
                            ax.set_xlim([datetime.date(2017, 3, 22), datetime.date(2017, 7, 18)])
                            ...
                            plt.savefig('/media/tweiss/Daten/data_AGU/test_kaska2/tau'+kkkk+'_'+k, bbox_inches = 'tight')
                            ...
                            plt.xlabel('Time')
                            plt.savefig('/media/tweiss/Daten/data_AGU/test_kaska/jose_unc08', bbox_inches = 'tight')
                            plt.close()
                            pdb.set_trace()

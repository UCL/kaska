...
common_computations = namedtuple(
    'common_computations', 'y y_unc y_fwd dH_fwd diff prior_cost dprior_cost')


class CostWrapper(object):
    def __init__(self, time_grid, current_data, gamma, emu, mu_prior,
                 c_prior_inv):
    ...
    def calc_cost(self, x, *args):
        ...
        idx = np.argmin(
            np.abs(np.array(self.doy_obs)[:, None] - np.array(self.time_grid)),
            axis=1)
        obs_cost = 0.

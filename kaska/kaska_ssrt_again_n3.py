...
def do_one_pixel_field(data_field, vv, vh, lai, theta, time, sm, sm_std, sr, sr_std, height, orbits, unc):
    ...
    for jj in range(len(vv)):
        ...
        if jj <= 2*n:
            orbit_mask[:2*n+1] = True
        elif jj > len(vv)-2*n:
            orbit_mask[jj-2*n:] = True
        else:
            orbit_mask[jj-n:jj+n] = True
        # pdb.set_trace()
        ...
        if jj <= int(n/2):
            coefs.append(posterious_coef[jj])
            sms.append(posterious_mv[jj])
            times.append(otime[jj])
        elif jj > len(vv)-2*n:
            coefs.append(posterious_coef[n])
            sms.append(posterious_mv[n])
            times.append(otime[n])
        else:
            coefs.append(posterious_coef[n])
            sms.append(posterious_mv[n])
            times.append(otime[n])

...
def test_wcm():
    ...
    A, B, C, V1, V2, sigma_soil \
        = -12,  0.05, 0.02, np.ones(2)*4, np.ones(2)*4, np.ones(2)*0.1
...
def test_optimiser():
    ...
    def cost_nolai(xx, svh, svv, lai, theta):
        return cost(np.concatenate([xx[:6], lai, lai, x[-2:]]),
                    svh, svv, theta)

    def cost_nolai_jac(xx, svh, svv, lai, theta):
        return cost_jac(np.concatenate([xx[:6], lai, lai, x[-2:]]),
                        svh, svv, theta)

    def cost_nolai_hess(xx, svh, svv, lai, theta):
        return cost_hess(np.concatenate([xx[:6], lai, lai, x[-2:]]),
                         svh, svv, theta)

    x0 = np.array([-10, 0.1, 1, -10, 0.1, 1, 0.2, 0.2])

    retval = scipy.optimize.minimize(cost_nolai, x0,
                                     args=(svh, svh, lai, theta),
                                     jac=cost_nolai_jac,

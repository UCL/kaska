...
def lookup():
    ...
    mv = np.arange(0.01, 0.5, 0.01)
    coef = np.arange(0.1,2,0.1)
    lai = np.arange(0.1, 5, 0.1)
    theta = np.arange(35,45,1)
    ...
    for r in mv:
        for rr in coef:
            for rrr in lai:
                for rrrr in h:
                    for rrrrr in theta:
                        x,y = ssrt_jac_(r, rr, rrr, rrrr, rrrrr)
                        hm[(r,rr,rrr,rrrr,rrrrr)] = x
    return hm

def lookup2(mv,coef,lai,theta,h,vv):
    hm = {}
    for r in mv:
        x,y = ssrt_jac_(r, coef, lai, h, theta)
        xx = 10*np.log10(x)
        xxx = abs(xx) - abs(vv)
        hm[r] = abs(xxx)
    return hm
                ...
                for kkkk in field:
                    for k in surface_list:
                        for kk in canopy_list:
                            ...
                            final_sm = [None] * len(height)
                            for i in range(len(height)):
                                mv = np.arange(0.01,0.4,0.05)
                                hm = lookup2(mv,coef[i],lai[i],theta[i],height[i],vv[i])
                                final_sm[i] = min(hm, key=hm.get)
                            pdb.set_trace()
